...
def macro_pill(sentiment: str) -> str:
    ...
    if "neutral" in s:
        return "pill-blue"
    return "pill-amber"   # mildly risk-on


# ── HTML template ─────────────────────────────────────────────────────────────

def build_html(br: dict, co: dict, co_pm: dict, iran: dict, macro: dict) -> str:
    # ── Brazil ────────────────────────────────────────────────────────────────
    ...
    m_vix  = macro.get("vix", "?")
    m_top  = macro.get("top_desk", "N/A")
    m_nl   = macro.get("n_long", "?")
    m_date = fmt_date(macro.get("as_of", "")) if macro.get("as_of") else "N/A"
    m_pill = macro_pill(m_sent)
...
.card-type-badge.macro {{ background: rgba(201,150,42,0.12); color: #e0b558; border-color: rgba(201,150,42,0.24); }}
    ...
    <a class="card card-macro" href="ll_macro.html">
      ...
      <div class="snapshot">
        <div class="snapshot-header">
          ...
          <span class="status-pill {m_pill}">{m_sent}</span>
        </div>
        <div class="snapshot-line">Sentiment dial <strong>{m_dial}/100</strong> &nbsp;&middot;&nbsp; VIX <strong>{m_vix}</strong></div>
        <hr class="snapshot-divider">
        <div class="snapshot-line">Top edge <strong>{m_top}</strong> &nbsp;&middot;&nbsp; Net long <strong>{m_nl}</strong> desks</div>
      </div>
      <div class="card-footer">
        <span class="card-updated">Updated {m_date}</span>
